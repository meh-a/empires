...
let screenShake  = 0;      // 0-1, decays over time
...
//  DAMAGE HELPERS
// ═══════════════════════════════════════════════════
function spawnDmgNumber(wx, wy, dmg, color) {
  dmgNumbers.push({ wx, wy: wy - 0.6, dmg: String(dmg), life: 1.0, color });
}

function _dmgVillager(v, dmg) {
  v.hp = Math.max(0, v.hp - dmg);
  v._hitFlash = 1.0;
  spawnDmgNumber(v.x, v.y, dmg, '#ff5555');
...
function _dmgBuilding(b, dmg) {
  ...
  spawnDmgNumber(b.tx + 0.5, b.ty, dmg, '#ff8833');
  if (b.hp <= 0) {
...
function _dmgEnemyUnit(eu, dmg) {
  ...
  eu._hitFlash = 1.0;
  spawnDmgNumber(eu.x, eu.y, dmg, '#f0d040');
...
function _dmgTC(dmg) {
  ...
  spawnDmgNumber(townCenter.tx + 0.5, townCenter.ty, dmg, '#ff5555');
  screenShake = Math.min(1.0, screenShake + 0.4);
  if (townCenter.hp <= 0) {
    gameState = 'defeat';
...
function updateCombat(dt) {
  ...
  // Decay attackAnim and hitFlash for all units
  for (const v of villagers) {
  ...
  // Decay screen shake and advance floating numbers
